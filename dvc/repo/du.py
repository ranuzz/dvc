...
def _get_dvc_file_size(fs, path):
    try:
        metadata = fs.metadata(path)
        if len(metadata.outs) > 0:
            return metadata.outs[0].meta.size
    except:
        return 0
    return 0
...
def _get_file_size(fs, path):
    ...
    if not is_dvc_file(path):
        return _get_dvc_file_size(fs, path) or fs.getsize(path)
    return _get_dvc_file_size(fs, path.split('.')[0])
